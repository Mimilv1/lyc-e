...
liste_variable = [longeur - (longeur * 0.057), hauteur - (hauteur * 0.1), pi, 9, 0, False, False, 0, 15]
# x ballon ,y ballon , x ballon 2,y ballon 2,angle ballon ,vitesse ballon au début,compteur pour attendre en miliseconde
# pour créer trajectoire,bool de si true stope la boucle,bool de pannier mis récemment, nb pannier
...
def changement_etat():
    ...
    if round(angle) < 0:  # test de si l'angle est trop petit
        ...
        angle = (vballon[2] * 180) / pi
    angle_rad = vballon[2]  # affectation dans des variables
    vitesse = vballon[3]  # affectation dans des variables
    vitesse_pos = abs(vitesse)  # affectation dans des variables
    vecteur[0] = vitesse_pos * (-sin(angle_rad))  # déplacement x
    vecteur[1] = vitesse_pos * (cos(angle_rad))  # déplacement y
    if 360 > angle >= 270:  # test dans quelle situation d'angle on est
        ...
        vballon[2] = vballon[2] - 0.005 - (cos(angle_rad) / cos(angle_rad)) * 0.005
        vballon[3] = vballon[3] + vballon[3] * 0.01
    elif round(angle) == 180 and round(yballon) < (hauteur - (hauteur / 10) - 0.03):
        ...
        if (round(vballon[3] * 10)) / 10 == 0:  # test de si la vitesse est égalle à 0 pour changer l'angle
            vballon[2] = 0
            angle_rad = vballon[2]
            vecteur[1] = vitesse_pos * (sin(angle_rad))
...
while True:
    ...
    trajectoire()  # on met un point noir sur la position du ballon
    anti_lag()  # affichage de la vitesse et l'angle actuelle du ballon
